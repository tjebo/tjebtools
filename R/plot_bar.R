...
plot_bar <- function (breaks, palette = 'Greys',
  ...
  if (!missing(colors))
    ...
    cbar_df = data.frame(stringsAsFactors = FALSE, y = breaks,
                         yend = c(breaks[-1], NA), color = as.character(1:n_breaks))[-n_breaks,]
    xmin = 1 - width/2
    xmax = 1 + width/2
    cbar_plot <-ggplot(cbar_df,
    ...
    if (any(inf_breaks == 1)) {
      firstv = breaks[1]
      polystart = data.frame(x = c(xmin, xmax, 1), y = c(rep(firstv,
                                                             2), firstv - diff(r_breaks) * triangle_size))
      plotcolors = plotcolors[-1]
      cbar_plot = cbar_plot + geom_polygon(data = polystart,
                                           aes(x = x, y = y), show.legend = FALSE, inherit.aes = FALSE,
                                           fill = colors[1], color = border_color)
    }
    if (any(inf_breaks > 1)) {
      lastv = breaks[n_breaks]
      polyend = data.frame(x = c(xmin, xmax, 1), y = c(rep(lastv,
                                                           2), lastv + diff(r_breaks) * triangle_size))
      plotcolors = plotcolors[-length(plotcolors)]
      cbar_plot = cbar_plot + geom_polygon(data = polyend,
                                           aes(x = x, y = y), show.legend = FALSE, inherit.aes = FALSE,
    ...
    if (!is.null(legend_title)) {
      cbar_plot <- cbar_plot + annotate(geom = "text", x = legend_position,
    ...
    cbar_df = data.frame(stringsAsFactors = FALSE, y = val_br[-length(val_br)],
    ...
    cbar_plot <- cbar_plot +
      geom_segment(data = data.frame(y = y_seg, yend = y_seg),
                   aes(y = y, yend = yend),
                   size = breaksize,
                   x = x - 0.05 * mul * spacing_scaling,
                   xend = xend, inherit.aes = FALSE) +
